...
import "package:jaspr_content/components/_internal/file_tree_icons.dart";
  ...
  static List<StyleRule> get styles => [
    css(".file-tree", [
      ...
      css("ul").styles(
        ...
        border: Border.only(
          left: BorderSide.solid(width: 1.px, color: Color.variable(FileTreeTheme._iconVariable)),
        ),
        listStyle: ListStyle.none,
      ),
      css("> ul").styles(padding: Padding.zero, margin: Margin.zero, border: Border.none),
      css("li", [
        css("&").styles(margin: Margin.symmetric(vertical: 0.125.rem)),
        css("&.file").styles(
        ...
        css("&.directory", [
          css("> details", [
            ...
            css("> summary", [
              ...
              css("&::marker").styles(color: Color.variable(FileTreeTheme._iconVariable)),
              css("&:hover", [
                css(
                  "&",
                ).styles(cursor: Cursor.pointer, color: Color.variable(FileTreeTheme._highlightVariable)),
                css("svg").styles(color: Color.variable(FileTreeTheme._highlightVariable)),
                css(
                  ".tree-entry-name[data-highlighted] svg",
                ).styles(color: Color.variable(FileTreeTheme._backgroundVariable)),
                css("~ul").styles(
                  border: Border.only(
                    left: BorderSide(color: Color.variable(FileTreeTheme._highlightVariable)),
                  ),
                ),
              ]),
            ]),
      ...
      css(".tree-entry", [
        ...
        css(".tree-entry-name", [
          css("&").styles(display: Display.inlineFlex, alignItems: AlignItems.center),
          css("&[data-highlighted]", [
            ...
            css("svg").styles(color: Color.variable(FileTreeTheme._backgroundVariable)),
          ]),
          css("&[data-placeholder]").styles(color: Color.variable(FileTreeTheme._iconVariable)),
        ]),
        css(".comment").styles(
          padding: Padding.only(left: 1.5.rem),
          color: Color.variable(FileTreeTheme._iconVariable),
...
class _FileTree extends StatelessComponent {
  const _FileTree({super.key, required this.items});
  ...
  Component build(BuildContext context) {
    return div(classes: "file-tree not-content", [
      ul([for (final item in items) buildFileTreeItem(item)]),
    ]);
  }
    ...
    if (item.children.isNotEmpty) {
      return li(classes: "directory", [
        details(open: item.isOpen, [
          summary([buildFileTreeEntry(item)]),
          ul([for (final child in item.children) buildFileTreeItem(child)]),
        ]),
      ]);
    }
  ...
  Component buildFileTreeEntry(FileTreeItem item) {
    ...
    return span(classes: "tree-entry", [
      span(
        ...
        [if (iconName != null) buildIcon(iconName), Component.text(isPlaceholder ? "…" : item.name)],
      ),
...
class FileTreeTheme extends ThemeExtension<FileTreeTheme> {
  FileTreeTheme({this.backgroundColor, this.textColor, this.iconColor, this.highlightColor, this.radius});
